...
const handleDrive = \`  const handleDriveUpload = async (fav: VideoData) => {
    if (!fav.rawVideoSrc) {
...
const uploadButton = \`                            <div className="absolute top-2 right-2 z-30 opacity-0 group-hover/thumb:opacity-100 transition-opacity flex flex-col gap-2">
                              <button onClick={(e) => { e.stopPropagation(); handleDriveUpload(fav); }} className="thumb-action p-1.5 bg-black/60 hover:bg-vault-accent text-white rounded shadow-lg backdrop-blur-md transition-all hover:scale-110" title="Save to Google Drive">
                                <UploadCloud size={12} />
                              </button>
