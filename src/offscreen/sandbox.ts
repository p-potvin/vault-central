...
window.addEventListener('message', async (event) => {
    ...
    const reply = (payload: object, transfer?: Transferable[]) => {
        if (transfer?.length) {
            src?.postMessage(payload, '*' as any, transfer);
        } else {
            src?.postMessage(payload, '*' as any);
        }
    };
    ...
    if (msg.type === 'vc_process') {
        ...
        try {
            const outBytes = await processVideo(ffmpeg, videoBytes as ArrayBuffer, duration as number);
            if (outBytes) {
...
async function processVideo(fm: FFmpeg, videoBytes: ArrayBuffer, duration: number): Promise<ArrayBuffer | null> {
    const ts = Date.now();
    ...
    try {
        ...
        if (duration <= 20) {
            ...
            for (let i = 0; i < numSegments; i++) filterParts.push(`[v${i}]`);
        ...
        try { await fm.deleteFile(inputName); } catch { /* ignore */ }
        try { await fm.deleteFile(outputName); } catch { /* ignore */ }
    }
}
