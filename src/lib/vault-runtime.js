...
export function getUnlockedVault() {
    return unlocked;
}
 ...
 * cleared.
 */
async function migrateLegacyPreviewsAndProvision(legacyPin, legacyRecords) {
    ...
    for (const record of legacyRecords) {
        try {
            const plain = await decryptLegacyRecord(record, legacyPin);
...
export async function encryptForStorage(plain) {
    if (!unlocked)
...
export async function decryptFromStorage(envelopeBytes) {
    if (!unlocked)
        throw new Error('Vault is locked');
 ...
 */
export async function savePreviewBlob(videoUrl, blob) {
    const settings = await getPinSettings();
    if (!settings.enabled) {
 ...
 */
export async function getPreviewBlob(videoUrl) {
    const record = await getPreviewRecord(videoUrl);
    if (!record)
        return null;
    if (record.encrypted)
        return null; // legacy — migration runs on next unlock
    if (record.blob instanceof Blob)
        return record.blob; // plaintext v2
    // Uint8Array → envelope
    if (!unlocked)
        return null;
    const plain = await decryptFromStorage(record.blob);
