 ...
 *
 * Ownership (Sun, 02 Aug 2026): this module used to live in the vaultwares-themes
 * submodule at security/crypto-vault.ts. vault-central is its only consumer, and
 * the submodule now ships the compiled .js only (the .ts was lost when a video
 * "*.ts" cleanup removed TypeScript sources), which broke `tsc` on the type-only
 * imports. Recovered verbatim from vaultwares-themes@6a54dd3^ and vendored here.
 * Verified byte-identical to the shipped crypto-vault.js after transpilation.
 * Any change to the envelope format must still follow the Tier 1 standard above.
 */
