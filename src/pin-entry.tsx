...
const PinPopup: React.FC = () => {
  ...
  if (!pinSettings.enabled) {
    return (
      <div className="w-[320px] p-6 bg-vault-bg text-vault-text flex flex-col items-center gap-4 border border-vault-border">
        <Icons.FingerprintIcon size={32} className="text-vault-accent" />
  ...
  if (!isLocked) {
    return (
      <div className="w-[320px] p-6 bg-vault-bg text-vault-text flex flex-col items-center gap-4 animate-in fade-in duration-200 border border-vault-border">
        <Icons.UnlockIcon size={32} className="text-vault-success" />
        <p className="text-[10px] font-mono uppercase tracking-[0.2em] font-bold text-vault-success">Vault Unlocked</p>
