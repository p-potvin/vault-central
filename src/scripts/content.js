...
if (browser.storage && browser.storage.onChanged) {
    browser.storage.onChanged.addListener((changes, areaName) => {
        if (areaName === 'local' && changes[STORAGE_KEYS.SAVED_VIDEOS]) {
            ...
            cachedSavedUrls = new Set(newValue.map((v) => v.url));
...
async function highlightVaultItems() {
    ...
    try {
        ...
        const links = document.querySelectorAll("a:not([data-vault-scanned])");
        let marked = 0;
        links.forEach(link => {
...
if (location.search.includes('__vaultTest=1')) {
    window.addEventListener('message', async (event) => {
        ...
        if (!msg || msg.__vaultTest !== 'request' || typeof msg.id !== 'string')
            return;
