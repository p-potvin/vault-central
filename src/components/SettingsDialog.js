...
import { DEFAULT_BACKUP_SETTINGS, getSavedVideos, saveVideos } from '../lib/storage-vault';
import { VideoDataSchema } from '../types/schemas';
export const SettingsDialog = ({ isOpen, onClose, onWipeVault, onImportSuccess, onShowToast, }) => {
    ...
    const handleRunFullBackup = async () => {
        ...
        try {
            ...
            onShowToast(`Full backup downloaded (${response.videos} items, ${response.previews} previews).`, "success");
            await refreshBackupSettings();
        }
        catch (err) {
